...
/**
 * Throws an {@link TypeError} if the given value parameter does not contain keys of given compere parameter.
 * @param {any} value 
 * @param {object} compere
 * @param {string} message Optional.
 * @throws {@link TypeError}
 */
Guard.NonSpecifikObject = (value, compere, message = undefined) => {
    if (typeof message !== 'string' && typeof message !== 'undefined') throw new TypeError(`The value of the message parameter got assigned a wrong type.`);
    if (typeof compere !== 'object' && typeof compere !== 'undefined') throw new TypeError(`The value of the compere parameter got assigned a wrong type.`);

    let keys = Object.getOwnPropertyNames(compere);

    for (let i = 0; i < keys.length; i++) {
        if (!keys[i] in value) throw new TypeError(message === undefined ? `Required value is not of specified object type.` : message);
    }
